...
export const { track, consent } = consentedTracking({
  // Or your own names, by hand. This entry point takes categories and never
  // learns a jurisdiction exists; `rulesFor` is one way to produce them.
...
**This entry point takes categories, not a region.** An earlier draft took `region: string` and
derived the categories from `rulesFor(region).defaults` itself. That coupled the convenient path to
the surveyed purposes: there was nowhere to declare a host's own name, and `needs: 'ads'` could
never resolve. Taking the object instead leaves both open, and a host that wants a jurisdiction to
pick its defaults writes one call:

```ts
categories: rulesFor('BR').defaults
```

The two packages therefore never meet. `@tracklane/consent` declares no dependency on
`@tracklane/consent-rules`, and the surveyed rules reach it only as a plain object the host passed.

The collapse into the vendor consent vocabulary follows the same shape. It defaults to
`toConsentState`, which reads the canonical `analytics` and `marketing` names and emits no key for
anything else, and a host with its own names supplies its own. A category nobody named produces no
signal, because widening silence into a grant would claim a permission never given.
