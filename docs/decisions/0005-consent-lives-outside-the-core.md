...
Three constraints on shipping it, all of them about being what it says it is:
...
**The disclaimer opens the README rather than sitting at the bottom of it.** The npm page is where
somebody decides to install, so that is where it belongs: a survey, not legal advice, read the
sources, and where this and the law disagree the law is right. A region nobody surveyed returns the
strictest configuration in the map rather than a guess.
