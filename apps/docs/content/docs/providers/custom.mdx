...
A wrapped provider is outside the conformance suite, with the same freedom and the same
responsibility a hand-written vendor call carries. And if two people write the same wrap, the
shipped provider is wrong: tell us, and it stops being your problem.
