...
- **A 200 is not a delivered event.** Meta answers 200 with a `messages` array when it took the
  ...
  can log.
- **A 400 says why, and the reason is in the body.** Unlike the warnings above, a refusal is
  explained in the response: `error.code`, `error_subcode` and a message naming the parameter.
  That body reaches you on `error.cause.body`; see [Why the vendor
  refused](/docs/installation#why-the-vendor-refused), including why every one of them lands in a
  single Sentry issue until you fingerprint them apart.
