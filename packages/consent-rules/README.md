...
guidance, written by software engineers. It is dated, it carries the source it was read from, and
it will go out of date.

It tells you what a regime appears to require. It cannot tell you whether your particular site, in
your particular business, is compliant. If the answer matters, and it usually does, have somebody
qualified read it. The authority that fines you is not a party to this licence, and the controller
of the data is you.
...
`BR`, `DE` and `US-CA`, plus the strict fallback for everywhere else. That is a starting point, not
a claim of global coverage: a visitor in France or Japan currently gets opt-in with everything
denied, which is conservative and possibly stricter than that jurisdiction requires.
