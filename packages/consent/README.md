...
gate and never will: it forwards a declaration to each vendor that documents a command for it, and
nothing more.
...
The category names are yours. To let a jurisdiction pick the defaults, pass
`rulesFor(region).defaults` from
[`@tracklane/consent-rules`](https://www.npmjs.com/package/@tracklane/consent-rules): this package
takes an object and never learns geography exists.
...
open rather than arrive as a field somebody added. A policy change is a new cookie name, which asks
everybody again.
